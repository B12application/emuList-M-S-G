...
// The exact SVG that generated concept_2_capsule_core.png
const exactConcept2Svg = `<svg viewBox="0 0 512 512" width="512" height="512" xmlns="http://www.w3.org/2000/svg">
  <defs>
...
async function renderExact() {
  const buf = Buffer.from(exactConcept2Svg);
  await sharp(buf).resize(512, 512).png().toFile('scratch/c2_512.png');
  await sharp(buf).resize(192, 192).png().toFile('scratch/c2_192.png');
  await sharp(buf).resize(64, 64).png().toFile('scratch/c2_64.png');
  await sharp(buf).resize(32, 32).png().toFile('scratch/c2_32.png');
  console.log('Exact C2 rendered!');
}
