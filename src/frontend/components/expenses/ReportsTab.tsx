      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-stone-50/50 dark:bg-zinc-800/30 rounded-[2rem] p-8 border border-stone-100 dark:border-zinc-800 flex flex-col justify-center items-center text-center space-y-6">
          ...
          <button
            ...
            className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 shadow-lg ${
              usage.count >= usage.limit 
              ? 'bg-stone-200 dark:bg-zinc-800 text-stone-400 cursor-not-allowed' 
              : 'bg-stone-900 dark:bg-white text-white dark:text-stone-900 hover:scale-105 active:scale-95 shadow-stone-900/10 dark:shadow-white/10'
            }`}
          >
          ...
          <div className="w-full max-w-[200px] space-y-2">
            <div className="flex justify-between text-[9px] font-black uppercase tracking-widest text-stone-400">
              ...
              <span className={usage.count >= usage.limit ? 'text-red-500' : 'text-stone-600 dark:text-stone-300'}>
                {usage.count} / {usage.limit}
              </span>
            </div>
            <div className="h-1.5 w-full bg-stone-100 dark:bg-zinc-800 rounded-full overflow-hidden">
              <div 
                className={`h-full transition-all duration-500 rounded-full ${
                  usage.count >= usage.limit ? 'bg-red-500' : 'bg-stone-900 dark:bg-white'
                }`}
