...
import { FaFire, FaPlus, FaCheck, FaSpinner, FaStar, FaFilm, FaCalendarAlt, FaClock, FaChevronLeft, FaChevronRight } from 'react-icons/fa';
...
export default function HomeTrendingRail({ onAdded, onSelect }: HomeTrendingRailProps) {
    ...
    const handleCardClick = async (item: TMDBMovieResult) => {
        ...
        const toastId = toast.loading(language === 'tr' ? 'Detaylar yükleniyor...' : 'Loading details...');
        try {
    ...
    useEffect(() => {
        const fetchTrending = async () => {
            setLoading(true);
            try {
                const results = await getTMDBTrending(mediaType, timeWindow);
                setItems(results.slice(0, 12)); // Show top 12 trending items
            } catch (error) {
    ...
    const handleQuickAdd = async (item: TMDBMovieResult) => {
        ...
        const toastId = toast.loading(language === 'tr' ? `${title} ekleniyor...` : `Adding ${title}...`);

        try {
            // 1. Check if already exists in collection
            const q = query(
            ...
            if (!snapshot.empty) {
                toast.error(language === 'tr' ? 'Bu içerik zaten listenizde ekli!' : 'Already in your list!', { id: toastId });
                setAddedIds(prev => ({ ...prev, [itemIdStr]: true }));
                return;
            }

            // 2. Fetch full details from TMDB (for IMDb ID, runtime, genres, total seasons)
            const details = await getTMDBDetails(item.id, mediaType);
            ...
            toast.success(language === 'tr' ? 'Koleksiyona eklendi!' : 'Added to collection!', { id: toastId });
            setAddedIds(prev => ({ ...prev, [itemIdStr]: true }));
            onAdded();
        } catch (error) {
            console.error('Error in quick add:', error);
            toast.error(language === 'tr' ? 'Ekleme başarısız oldu.' : 'Failed to add item.', { id: toastId });
        } finally {
    ...
    return (
        <section className="mt-12 bg-white/20 dark:bg-zinc-950/20 backdrop-blur-md border border-slate-200/50 dark:border-zinc-800/40 rounded-3xl p-6 shadow-sm">
            {/* Header Area */}
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
                <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-amber-100 text-amber-500 dark:bg-amber-950/55 dark:text-amber-400 shadow-inner">
                        <FaFire className="text-lg animate-pulse" />
                    </div>
                    <div>
                        <h2 className="text-xl font-bold tracking-tight text-slate-900 dark:text-white">
                            {language === 'tr' ? 'Dünya Trendleri' : 'Global Trends'}
                        </h2>
                        <p className="text-xs text-slate-500 dark:text-zinc-400">
                            {language === 'tr' ? 'Bu ay en çok izlenen popüler içerikler' : 'Most watched popular content worldwide'}
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    {/* Media Type Tabs */}
                    <div className="flex bg-slate-100 dark:bg-zinc-900/80 p-1 rounded-xl border border-slate-200/60 dark:border-zinc-850">
                        <button
                        ...
                        >
                            {language === 'tr' ? 'Trend Filmler' : 'Trending Movies'}
                        </button>
                        <button
                        ...
                        >
                            {language === 'tr' ? 'Trend Diziler' : 'Trending Series'}
                        </button>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 ml-1">
                        <button
                            type="button"
                            onClick={() => scroll('left')}
                            className="flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg border border-slate-205 text-slate-500 transition hover:border-slate-300 hover:bg-slate-50 hover:text-slate-700 dark:border-zinc-800 dark:text-zinc-550 dark:hover:border-zinc-700 dark:hover:bg-zinc-800 dark:hover:text-zinc-300"
                        >
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-20 gap-3">
                    ...
                    <span className="text-xs text-slate-400 dark:text-zinc-500 font-medium">
                        {language === 'tr' ? 'Trend içerikler getiriliyor...' : 'Fetching trending content...'}
                    </span>
                </div>
            ) : items.length === 0 ? (
                <div className="text-center py-12 border border-dashed border-slate-200 dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-950/20 rounded-2xl">
                    <p className="text-sm text-slate-500 dark:text-zinc-400 font-medium">
                        {language === 'tr' ? 'Veri bulunamadı.' : 'No data found.'}
                    </p>
                </div>
            ) : (
                    ...
                    {items.map((item, idx) => {
                        ...
                        return (
                            <motion.div
                                ...
                                className="group relative flex flex-col w-[200px] sm:w-[220px] shrink-0 bg-white dark:bg-zinc-900 border border-slate-200/70 dark:border-zinc-800/80 rounded-2xl overflow-hidden shadow-sm hover:shadow-lg hover:border-amber-400/50 dark:hover:border-amber-500/50 transition-all snap-start cursor-pointer"
                            >
                                {/* Poster Image */}
                                <div className="relative aspect-[2/3] overflow-hidden bg-slate-100 dark:bg-zinc-800">
                                    ...
                                    <button
                                        ...
                                        title={addedIds[itemIdStr] ? 'Zaten Ekli' : 'Listeme Ekle'}
                                    >
                                        {addingIds[itemIdStr] ? (
