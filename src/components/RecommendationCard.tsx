...
export default function RecommendationCard({ item, typeLabel, refetch }: RecCardProps) {
  ...
  const getIcon = () => {
    ...
    if (typeLabel === 'Kitap') return <FaBook className="text-purple-500" />;
    return <FaGamepad className="text-amber-500" />;
  };

  // typeLabel'i çevirmek için helper
  const getTranslatedTypeLabel = () => {
    if (typeLabel === 'Film') return t('media.movie');
    if (typeLabel === 'Dizi') return t('media.series');
    if (typeLabel === 'Oyun') return t('media.game');
    if (typeLabel === 'Kitap') return t('media.book');
    return typeLabel;
  };
              ...
              <div className="relative h-64 w-full overflow-hidden rounded-t-3xl bg-gray-100 dark:bg-gray-900">
                ...
                <div className="absolute top-3 left-3 flex gap-2">
                  <span className="px-3 py-1 bg-black/60 backdrop-blur-md text-white text-xs font-bold rounded-full flex items-center gap-1">
                    {getIcon()} {getTranslatedTypeLabel()}
                  </span>
                </div>
