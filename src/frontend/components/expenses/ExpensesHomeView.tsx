...
const ExpensesHomeView: React.FC<ExpensesHomeViewProps> = ({
  ...
  return (
    <div className="space-y-6">
      {!isTrashView && (
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            {[
              { label: 'TOPLAM GELİR', value: totalIncomeAmount, color: 'text-emerald-600 dark:text-emerald-400' },
              { label: 'TOPLAM GİDER', value: totalExpenseAmount, color: 'text-rose-600 dark:text-rose-400' },
              { label: 'NET DURUM', value: Math.abs(netAmount), color: netAmount >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400', prefix: netAmount < 0 ? '-' : '+' },
              { label: 'TÜM ZAMANLAR', value: totalLifetimeAmount, color: 'text-blue-600 dark:text-blue-400' }
      ...
      {/* List Container */}
      <div className={`bg-stone-50/50 dark:bg-zinc-800/30 rounded-[2.5rem] border border-stone-100 dark:border-zinc-800 flex flex-col overflow-hidden ${isTrashView ? 'min-h-[500px]' : ''}`}>
        ...
        <div className="flex-1 overflow-y-auto max-h-[600px] custom-scrollbar p-3">
          {filteredExpenses.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-stone-300 dark:text-zinc-700">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.3em]">{isTrashView ? 'ÇÖP KUTUSU BOŞ' : t('expenses.noExpenses')}</p>
            </div>
          ) : (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      {!isTrashView ? (
                        ...
                        <>
                          <button
                            onClick={() => handleEditClick(expense)} // Restore function passed as handleEditClick
                          ...
                          <button
                            onClick={() => handleDeleteExpense(expense.id)} // Hard delete function passed as handleDeleteExpense
