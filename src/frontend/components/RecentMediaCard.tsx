...
export default function RecentMediaCard({ item, onClick }: RecentMediaCardProps) {
  ...
  const getTypeIcon = () => {
    switch (item.type) {
      ...
      default: return <FaFilm className="text-stone-500" />;
    }
  };
  ...
  return (
    <div
      onClick={onClick}
      className="group relative flex-shrink-0 w-80 bg-stone-50 dark:bg-zinc-800/80 backdrop-blur-sm border border-stone-200 dark:border-zinc-700 rounded-2xl overflow-hidden cursor-pointer hover:shadow-xl hover:border-sky-200 dark:hover:border-sky-900 transition-all duration-300 transform hover:-translate-y-1"
    >
        ...
        <div className="flex-1 p-4 flex flex-col justify-between">
          <div>
            ...
            <div className="flex justify-between items-start mb-2">
              <div className="p-1.5 bg-stone-100 dark:bg-zinc-700/50 rounded-lg">
                {getTypeIcon()}
              </div>
            ...
            <h3 className="font-bold text-stone-900 dark:text-white leading-tight line-clamp-2 mb-1">
              {item.title}
            </h3>
          ...
          {/* Alt Bilgi: Tarih */}
          <div className="flex items-center gap-1.5 text-[10px] font-medium text-stone-400 uppercase tracking-wide mt-3 pt-3 border-t border-stone-200 dark:border-zinc-700">
            <FaCalendarAlt />
